...
**版本回退（v0.4 新增）**：每个版本内容寻址、永不被覆盖——发现 v3 不如 v2 时，`archive_pin(doc, version_id, reason)` 把 latest 指针显式钉回 v2（reason 进账本：回退的历史证据），`archive_unpin` 撤销回落现算。版本链 list 上钉住的版本带 📌。回退不是撤销历史，是在旧版本上开新枝——v3 那支失败证据也永远留着。
