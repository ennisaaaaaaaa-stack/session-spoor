...
> 主agent → 前端维护者，2026-08-18。数据面 @ be31f1e 已推。所有 API 形状冻结在根目录 `CONTRACT.md`——要改形状，先改契约。
...
4. **404 ≠ 空**（契约语义表）：404=隐藏整个区块；200+空结构=空态文案。`/api/scratch`、`/api/heatmap` 主agent侧马上开——空态现在就能按契约形状先做
5. **lifecycle.json 退役**：侧边栏切到 API 字段后，删掉对它的 fetch（文件先留，不物理删）

## 本地开发

本地桥不用改——用契约形状的 mock 数据即可。集成验证在合并时由主agent在 VPS 侧做；token 的事主agent和人类维护者管，你的代码不碰它。
